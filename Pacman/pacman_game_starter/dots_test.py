...
def test_eat():
    ds = Dots(600, 600, 150, 450, 150, 450)
    assert len(ds.eat(0, ds.TH)[0]) == ds.WIDTH//ds.SPACING - 1
    assert len(ds.eat(0, ds.TH)[1]) == ds.WIDTH//ds.SPACING + 1
    assert len(ds.eat(0, ds.TH)[2]) == ds.WIDTH//ds.SPACING + 1
    assert len(ds.eat(0, ds.TH)[3]) == ds.WIDTH//ds.SPACING + 1
    assert ds.top_row[0].x == ds.SPACING
    assert ds.top_row[-1].x == ds.WIDTH - ds.SPACING
    assert len(ds.eat(ds.LV, ds.BH)[1]) == ds.WIDTH//ds.SPACING
    assert len(ds.left_col) == ds.WIDTH//ds.SPACING
